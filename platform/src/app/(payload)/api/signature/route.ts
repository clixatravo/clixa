...
import { empreinteDesTermes, memeNom, mentionValable, preuve } from "@/lib/signature";

/**
 * Le participant signe son contrat en ligne.
 *
 * ── Ce que cette route accepte comme signature ──────────────────────────────
 * Le nom du dossier recopié, et la mention « Lu et approuvé » que le contrat
 * exige. Rien d'autre : une case cochée seule se coche pour n'importe qui, et
 * un tracé à la souris n'ajoute aucune force — il ajoute une image.
 *
 * ⚠️ La force ne vient pas du geste mais de ce qu'on garde autour : la date,
 * l'adresse IP, le navigateur et l'empreinte des termes au moment de signer.
 * Sans l'empreinte, la première objection serait « le document a changé
 * depuis », et elle serait imparable.
 *
