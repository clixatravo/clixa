...
export function generateStaticParams() {
  return getSpecialisations().map((s) => ({ slug: s.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const s = getSpecialisation(slug);
  if (!s) return {};
...
export default async function PageSpecialisation({ params }: Props) {
  const { slug } = await params;
  const spec = getSpecialisation(slug);
  if (!spec) notFound();

  const programmes = getProgrammesParSpecialisation(spec.slug);
  const nbSessions = programmes.reduce((t, p) => t + getSessions(p.slug).length, 0);
  ...
  const index = getSpecialisations().findIndex((s) => s.slug === spec.slug) + 1;
